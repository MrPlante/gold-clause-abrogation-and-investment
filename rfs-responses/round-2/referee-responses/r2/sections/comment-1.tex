...
\end{tabular}
\\[4pt]
\begin{minipage}{0.92\textwidth}\footnotesize \textit{Note.} Cumulative raw returns of the equal-weighted gold-exposed and non-exposed portfolios within size terciles formed on market capitalization at the end of 1932 (gold/non-gold firm counts: small 69/116, medium 51/133, large 57/127). ``Diff.'' is the gold-minus-non-gold differential in percentage points. The beta-adjusted differential subtracts $(\beta_G-\beta_N)\times R_m$, where $R_m$ is the value-weighted market return over the window and portfolio betas are equal-weighted averages of firm-level market betas estimated from daily returns over the full 1926--1940 sample (small: $\beta_G=0.93$, $\beta_N=0.90$; medium: $\beta_G=1.11$, $\beta_N=0.91$; large: $\beta_G=1.02$, $\beta_N=0.90$). $t$-statistics scale each differential by $\hat\sigma\sqrt{h}$, where $h$ is the number of trading days in the window and $\hat\sigma$ is the standard deviation of the corresponding daily differential over calm 1934 trading days, as in Table~1 of the paper.\end{minipage}
}
\end{center}
