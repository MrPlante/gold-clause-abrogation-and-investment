...
\begin{itemize}
\item \textbf{Stock-return analysis.} Following R2's suggestion, the paper now presents the stock market evidence in a table and four event figures. Table~1 reports raw returns for four portfolios---the value-weighted market, equal-weighted portfolios of firms with and without gold-clause exposure, and the gold-exposure-weighted portfolio---over each of the four key events, along with firm counts. Figures~3--6 plot all four series around each event. Over the Joint Resolution window, gold-exposed firms returned 30.9--31.4\% against 25.4\% for comparable non-exposed firms and 12.1\% for the market. A new Internet Appendix section examines every intermediate judicial event of the litigation (the first hearing, the lower-court rulings, and the first certiorari grant) and reports the corresponding return differentials, pooled and by firm size (Table~IA.21); none produced a significant differential. The same examination surfaced a significant differential in the November 1934 window, in which the second certiorari grant coincided with the midterm election; we now include this window among the main events (Table~1, Figure~4, and Section~3).
