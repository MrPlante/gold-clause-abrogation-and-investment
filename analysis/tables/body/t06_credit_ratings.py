...
def validate_against_manuscript(models: dict[str, object]) -> list[tuple[str, float, float]]:
    parsed = _parse_manuscript_table(MANUSCRIPT_BODY_TABLES / "table6_credit_ratings.tex")
...
def write_latex_table(models: dict[str, object], path: Path | None = None) -> Path:
    out = path or (MANUSCRIPT_BODY_TABLES / "table6_credit_ratings.tex")
...
def main() -> dict[str, object]:
    ...
    if failures:
        print(
            f"WARNING: Table 5 manuscript check differences "
