...
def _parse_manuscript_table(tex_path: Path) -> dict[str, list[dict]]:
    """Parse panel rows from table2_summary_stats.tex into structured dicts."""
...
def validate_against_manuscript(panels: dict) -> list[tuple[str, float, float]]:
    parsed = _parse_manuscript_table(MANUSCRIPT_BODY_TABLES / "table2_summary_stats.tex")
...
def write_latex_table(panels: dict, path: Path | None = None) -> Path:
    ...
    out = path or (MANUSCRIPT_BODY_TABLES / "table2_summary_stats.tex")
