...
from config import COEF_TOLERANCE, MANUSCRIPT_BODY_TABLES, MANUSCRIPT_BODY_TABLES
...
def validate_against_manuscript(rows: list[BondYearStats]) -> list[tuple[str, float, float]]:
    parsed = _parse_manuscript_table(MANUSCRIPT_BODY_TABLES / "table3_bond_stats.tex")
...
def write_latex_table(rows: list[BondYearStats], path: Path | None = None) -> Path:
    out = path or (MANUSCRIPT_BODY_TABLES / "table3_bond_stats.tex")
