...
Splits all panels by dind (I{d>0}), matching manuscript ia03_sum_stats_tilde_d.tex.
...
def validate_against_manuscript(panels: dict) -> list[tuple[str, float, float]]:
    parsed = _parse_manuscript_table(
        MANUSCRIPT_APPENDIX_TABLES / "ia03_sum_stats_tilde_d.tex"
    )
...
def write_latex_table(panels: dict, path: Path | None = None) -> Path:
    out = path or (MANUSCRIPT_APPENDIX_TABLES / "ia03_sum_stats_tilde_d.tex")
