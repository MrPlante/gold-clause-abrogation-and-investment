"""
Internet Appendix Table 2 — Summary statistics for tilde-d > 0 firms (tabapp:summary_I_0).

Uses ``d > 0`` (manuscript / Stata A14 first block). Panel B omits tilde-d indicator rows.
"""
...
TABLE_NOTES = (
    ...
    r"traded industrial firms with positive gold clause exposure ($\tilde{d} > 0$). "
    ...
    r"are the sum of corporate bonds, preferred shares, and bank debt. In Panels A and B, "
...
def validate_against_manuscript(panels: dict) -> list[tuple[str, float, float]]:
    parsed = _parse_manuscript_table(MANUSCRIPT_APPENDIX_TABLES / "ia04_summary_I_1.tex")
...
def write_latex_table(panels: dict, path: Path | None = None) -> Path:
    out = path or (MANUSCRIPT_APPENDIX_TABLES / "ia04_summary_I_1.tex")
...
def main() -> dict:
    ...
    if failures:
        print(
            f"IA Table 2 (summary_I_1) validation failed ({len(failures)}/{len(checks)} checks):\n"
            + "\n".join(failures[:20])
        )

    else:
        print(f"IA Table 2 (summary_I_1) — all {len(checks)} manuscript checks passed (tol={TOL})")
    for key in ("A", "B", "C"):
