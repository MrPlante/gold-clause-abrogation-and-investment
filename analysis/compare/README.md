...
2. **User packages** (installed automatically on first run if missing; or manually):
   ...
   ftools, compile
   ```
3. **Data:** `data/processed/firm_year_panel.dta` (Mete-built panel).
...
```bash
bash analysis/compare/run_compare.sh --fail-on-mismatch
```
...
bash analysis/stata/run_stata_do.sh analysis/compare/export_regressions.do
.venv/bin/python analysis/compare/export_python.py
.venv/bin/python analysis/compare/compare_results.py
```
