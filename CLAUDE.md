...
the coefficient-only aggregations (T8, IA.18).
...
All return series, event figures (manuscript Figures 3–6), and
tables (Table 1, IA.20, IA.21) are generated by a single pipeline
...
- All printed regression numbers come from the per-table reghdfe do-files
  ...
  pyfixest survives only for coefficient-only aggregations (T8, IA.18).
