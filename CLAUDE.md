...
| `pipeline/` | Data wrangling: raw sources -> A0-A3 intermediates -> `A4_merged.dta` (+ researchdb SQL build) |
...
| `data/` | Panel data (not versioned; see `data/README.md`): `A4_merged.dta` at the root, sources in `raw/`, A0–A3 in `intermediates/` (retired attic material now in `tmp/2026-07/21/attic/`) |
...
This regenerates all body and IA tables into `(removed mirror) output/` and also
...
stata-mp -b do analysis/stata/A12_controls_indyear.do
```
...
Built/rebuilt by `pipeline/sql/build_gold_claude_crsp.sql` (run
...
|---|---|---|
| Table 1 | `0_event_study.tex` | Event study (generated by `analysis/eventstudy/pipeline.py`) |
...
- Regressions that require exact Stata SEs use `analysis/stata/` do-files
  directly (IA.19 = `A12_controls_indyear.do`); the original Stata pipeline
