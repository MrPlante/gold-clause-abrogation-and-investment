...
Every table that prints reghdfe standard errors (T4-T7, IA.9-IA.16) is estimated by a per-table batch do-file in
...
Stata is missing or the do-file fails, the stage fails loudly. Figure 6
reads the Table 4 results CSV, so run `--stage table4` before `figures` if
the estimates changed. Do-files can also be run standalone from the
project root, e.g.:

```bash
cd /project7/splante/git/gold-clause-abrogation-and-investment
stata-mp -b do analysis/stata/table7_controls.do   # after the builder wrote its input
stata-mp -b do analysis/stata/ia16_controls_indyear.do
```

Logs land in `logs/` (gitignored). The former in-process vcov bridge and
its `USE_STATA_VCOV` switch are gone; pyfixest (CGM fix) remains only for
the coefficient-only aggregations (T8, IA.19).
...
All return series, event figures (manuscript Figures IA.1–IA.4), and
tables (Table 1) are generated by a single pipeline
...
`analysis/tables/appendix/iaNN_<topic>.py` via `--stage iaN`. Table 1 is
built by the `eventstudy` stage. IA numbers are still ASSIGNED by
...
`15_controls_extra.tex` was the stock-controls IA table, now IA.15; note also the 2026-08-30 renumbering of IA.9-IA.19 into first-mention order).
...
1. **reghdfe engine version matters for two-way-cluster SEs.** reghdfe 6
   ...
   under reghdfe 6 (all-missing e(V)): Table 4 col 7 and IA.15's ret-sd
...
2. **The old rc=-11 "segfault" on decile + sic2_year specs was a defect of
   ...
   do-files (`stata-mp -b`); if a big spec ever misbehaves, use the
   batch-do-file pattern, never an in-process call.
...
6. **Nothing under `data/` is ever committed.** Generated paper objects go
   ...
   controls-indyear Stata CSV now lives at `analysis/stata/ia16_controls_indyear.csv`).
...
- All printed regression numbers come from the per-table reghdfe do-files
  in `analysis/stata/` (engine pinned per table; committed results CSVs);
  pyfixest survives only for coefficient-only aggregations (T8, IA.19).
  The original Stata pipeline lives only in git history
  (`git checkout d54b0c3 -- code/legacy`)
