"""
Internet Appendix Table 0a — Summary statistics for d > 0 firms (tabapp:summary_d_1).

Uses contemporaneous exposure ``d_orig > 0`` (manuscript); Stata A14 uses ``d > 0``
and exports extra tilde-d rows not shown in the manuscript table.
"""
...
TABLE_NOTES = (
    ...
    r"traded industrial firms with positive contemporaneous gold clause exposure "
    r"($d > 0$). Panel A covers the pre-abrogation period (1926--1932) and Panel B "
    r"the legal uncertainty period surrounding the abrogation of the gold clause "
    r"(1933--1934). Long-term liabilities (LTL) are the sum of corporate bonds, "
    r"preferred shares, and bank debt. All variables are winsorized at the 0.5\% and "
    r"99.5\% levels within each year. See Appendix \ref{secapp:vars} for detailed "
...
def validate_against_manuscript(panels: dict) -> list[tuple[str, float, float]]:
    parsed = _parse_manuscript_table(MANUSCRIPT_APPENDIX_TABLES / "0a_summary_d_1.tex")
...
def write_latex_table(panels: dict, path: Path | None = None) -> Path:
    out = path or (REFACTOR_OUTPUT_TABLES_APPENDIX / "0a_summary_d_1.tex")
